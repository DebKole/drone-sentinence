...
- Uses OpenCV (MJPEG 800x600) for instant, non-blocking camera captures.
...
class CameraManager:
    def __init__(self):
        ...
        # FIXED: Explicitly use V4L2 to prevent GStreamer pipeline errors
        self.cam = cv2.VideoCapture(0, cv2.CAP_V4L2)
        
        # Match the ffmpeg parameters for format and size
        self.cam.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
        self.cam.set(cv2.CAP_PROP_FRAME_WIDTH, 800)
        self.cam.set(cv2.CAP_PROP_FRAME_HEIGHT, 600)
        self.cam.set(cv2.CAP_PROP_BUFFERSIZE, 1) # Keep buffer tiny to avoid stale frames
        
        # Warm up the sensor
        time.sleep(1)
        ret, _ = self.cam.read()
        if not ret:
            log("ERROR", "Failed to read from /dev/video0. Check camera connection!")

    def _capture_thread(self, folder_path, img_name):
        # Flush the buffer to ensure we get the frame from EXACTLY right now
        for _ in range(3):
            self.cam.grab()
        ...
        if ret:
            ...
            log("ERROR", f"Failed to capture {img_name}.jpg")

    def trigger(self, folder_path, img_name):
        """Fires the camera in a background thread so the drone doesn't freeze."""
